...
| サービス | 用途 | 備考 |
|----------|------|------|
| [Oracle Cloud (OCI)](https://www.oracle.com/jp/cloud/free/) | クラスター本体 | Always Free Tier で登録 |
| [Cloudflare](https://www.cloudflare.com/) | DNS 管理・証明書発行 | 独自ドメインを Cloudflare に移管または追加 |
| [GitHub](https://github.com/) | GitOps リポジトリ・OIDC 認証 | Organization または個人アカウント |
| [Slack](https://slack.com/) | Alertmanager 通知 | 任意（通知不要なら省略可） |
...
[default]
aws_access_key_id = xxx      # 下記手順で発行した顧客シークレットキーのアクセスキー
aws_secret_access_key = xxx  # 下記手順で発行した顧客シークレットキーのシークレット
```

`[default]` セクションの顧客シークレットキーの発行手順:

1. OCI コンソール → 右上のユーザーアイコン → **My profile**
2. 左メニュー → **Customer secret keys** → **Generate secret key**
3. Name に `terraform-backend` (任意) を入力 → **Generate secret key**
4. 表示された **Secret** を `aws_secret_access_key` に設定（この画面を閉じると再表示できません）
5. 生成されたキーの **Access Key** 列の値を `aws_access_key_id` に設定

> [!NOTE]
> この顧客シークレットキーは Terraform state の保存（Step 1 で作成したバケット）にのみ使用します。S3 Proxy 用（付録参照）とは用途が異なるため、別のキーとして発行してください。
...
EMAIL="your@example.com"        # Let's Encrypt 証明書登録用メールアドレス
...
# 変更を push (ArgoCD がこのファイルを参照するため push が必要)
git add gitops/core/s3-proxy/values.yaml
git commit -m "chore: configure s3-proxy bucket"
git push
```
...
適用後、`terraform/infra/` の **1つ上**のディレクトリ（`terraform/.kube.config`）に kubeconfig が生成されます。

> [!NOTE]
> `~/.kube/config` ではなく `terraform/.kube.config` として生成されるのは意図的な設計です。
> `terraform/config/` の Kubernetes / Helm / kubectl プロバイダーが同じファイルを相対パス `"../.kube.config"` で参照しているため、プロジェクト外に置くと `terraform/config` の `terraform apply` が失敗します。
> また、既存のクラスター設定（`~/.kube/config`）を上書きしないという利点もあります。

```bash
# クラスターへの接続確認 (terraform/infra/ から実行)
...
以下のコマンドはリポジトリルート（`oci-build-free-k8s/`）から実行します。

```bash
# ArgoCD の Pod 確認
kubectl --kubeconfig terraform/.kube.config -n argocd get pods

# ArgoCD UI へのポートフォワード
kubectl --kubeconfig terraform/.kube.config -n argocd port-forward svc/argocd-server 8080:80

# 初期管理者パスワードの取得
kubectl --kubeconfig terraform/.kube.config -n argocd get secret argocd-initial-admin-secret \
...
```bash
kubectl --kubeconfig terraform/.kube.config get certificate -A
kubectl --kubeconfig terraform/.kube.config get clusterissuer
```
...
for appset in core-helm-apps core-gitonly-apps; do
  kubectl --kubeconfig terraform/.kube.config -n argocd patch applicationset $appset \
    --type='json' \
...
```bash
# GitHub SSO でログイン
# --auth の値は Teleport の GitHub コネクタ名 (teleport/values.yaml の connectorName に合わせる)
tsh login --proxy teleport.<your-domain>:443 --auth=<TELEPORT_GITHUB_CONNECTOR_NAME> --user YOUR_GITHUB_USER teleport.<your-domain>
...
### `teleport-github-client-id` / `teleport-github-client-secret` — Teleport 用 GitHub OAuth App

Teleport の GitHub SSO に使用します。Dex 用とは**別の** OAuth App として作成してください。
...
| 項目 | 値 |
|------|----|
| Application name | `teleport-oci-k8s` (任意) |
| Homepage URL | `https://teleport.<your-domain>` |
| Authorization callback URL | `https://teleport.<your-domain>/v1/webapi/github/callback` |

3. **Register application** → 表示された **Client ID** を `teleport-github-client-id` に登録
4. **Generate a new client secret** → 表示された **Client Secret** を `teleport-github-client-secret` に登録
